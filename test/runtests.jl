...
@testitem "Test quaternary (4-arity) nodes and operators" begin
    ...
    # Define and declare a 4-ary operator
    my_sumprod(a, b, c, d) = a * b + c * d
    @declare_expression_operator(my_sumprod, 4)

    # Build operator enum with required lower-arity ops
    operators = OperatorEnum(
        4 => (my_sumprod,),
        3 => (),               # none needed but keep arity ordering explicit
        2 => (+, *, /, -),
        1 => (sin,),
    )
    ...
    expr = my_sumprod(v1, v2, v3, v4)

    # First-order partial derivative printing
    @test repr(D(expr, 1)) == "∂₁my_sumprod(v1, v2, v3, v4)"
    @test repr(D(expr, 2)) == "∂₂my_sumprod(v1, v2, v3, v4)"
    @test repr(D(expr, 3)) == "∂₃my_sumprod(v1, v2, v3, v4)"
    @test repr(D(expr, 4)) == "∂₄my_sumprod(v1, v2, v3, v4)"
